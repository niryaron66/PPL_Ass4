...
const applicativeEval = (exp: CExp, env: Env): Result<Value> => {
    ...
    isProcExp(exp) ? evalProc(exp, env) :  // define f1 lambda b +b c
...
const evalTraceExp = (exp: TraceExp, env : Env): Result<void> =>
    // complete this
{
const binding=applyEnvBdg(env,exp.var.var);
if(isOk(binding)) {
    const closure = applyEnv(env, exp.var.var);
    if (isOk(closure) && isClosure(closure.value))
        setFBinding(binding.value, makeTracedClosure(closure.value, exp.var.var))
    else
        return makeFailure("id is not function");
    return makeOk(void "tracedClosure");
}
return makeFailure("id dont exist")
}
