...
// HW3
export const makeTraceExp = (v: VarRef): TraceExp => ({tag: "TraceExp", var:v});
    // ccompleted.
...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    (params) => // (trace [f0,f1,f2])
        params.length !==1  ? makeFailure("Expression not fit to the form (trace id)") :
            mapv(makeOk(makeVarRef(params[0].toString())) , (v: VarRef) => makeTraceExp(v));
